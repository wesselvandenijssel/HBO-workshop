...
def standaardprijs(afstandKM):
    # Check of de afstand 0 of kleiner is
    if afstandKM <= 0:
        # Kost niets
        standaardprijs = 0
    elif afstandKM <= 50:
        # 0,80 cent per kilometer
        standaardprijs = 0.80 * afstandKM
    else:
        # 15 + 0.60 cent per kilometer
...
def ritprijs(leeftijd, weekendrit, afstandKM):
    # Check of iemand jonger dan 12 is of 65 jaar en ouder
    if  leeftijd < 12 or leeftijd >= 65:
        # Check of het een weekendrit is
        if weekendrit == False:
            # Dit GEEN weekendrit, dus is er 30% korting
            ritprijs = standaardprijs(afstandKM) / 100 * 70
        else:
            # Dit is een weekendrit, dus 35% korting
            ritprijs = standaardprijs(afstandKM) / 100 * 65
    # Check of iemand die ouder dan 12 jaar is en jonger dan 65 jaar tijdens een weekendrit
    elif weekendrit == True:
        # Het is een weekendrit, dus 40% korting
        ritprijs = standaardprijs(afstandKM) / 100 * 60
    else:
        # Het is een standaardrit, dus normale prijs
...
def development_code():
    ...
    # print("development printout:", ritprijs(11, False, 30))
    return
...
def test_standaardprijs():
    ...
    for test in testcases:
        __my_assert_args(standaardprijs, (test.distance,),
                         test.expected_output)


def test_ritprijs():
    case = collections.namedtuple(
        'case', 'age weekend distance expected_output')

    testcases = [case(11, True,  50, 26.0),  case(11, False,  50, 28.0),  case(11, True,  51, 29.64), case(11, False, 51, 31.92),
                 case(11, True, -51,  0.0),  case(11, False, -51,
                                                  0.0),  case(12, True,  50, 24.0),  case(12, False, 50, 40.0),
                 case(12, True,  51, 27.36), case(12, False,  51, 45.6),  case(
                     12, True, -51,  0.0),  case(12, False, -51, 0.0),
                 case(64, True,  50, 24.0),  case(64, False,  50, 40.0),  case(
        64, True,  51, 27.36), case(64, False, 51, 45.6),
        case(64, True, -51,  0.0),  case(64, False, -51,
                                         0.0),  case(65, True,  50, 26.0),  case(65, False, 50, 28.0),
        case(65, True,  51, 29.64), case(65, False,  51, 31.92)]

    for test in testcases:
        __my_assert_args(ritprijs, (test.age, test.weekend,
                         test.distance), test.expected_output)
